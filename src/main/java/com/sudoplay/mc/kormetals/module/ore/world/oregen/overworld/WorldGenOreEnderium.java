...
public class WorldGenOreEnderium extends
  ...
  public WorldGenOreEnderium(
      Kor kor,
      @KorJsonConfig(path = ModuleOre.Config.OreGenOverworld.CONFIG_PATH, file = "enderium.json") Config config
  ) {
    super(
        config,
        kor.get(BlockOre.class).getDefaultState().withProperty(BlockOre.TYPE, MetalType.Enderium)
    );
  }
}
