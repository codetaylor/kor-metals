 ...
 */
public class RecipeShapelessNuggetToIngotDelegate extends
    KorRegistrationDelegate {

  private TextConfigData config;

  @KorInject
  public RecipeShapelessNuggetToIngotDelegate(
  ...
  private boolean isRecipeEnabledInConfig(String name) {
    return this.config.getCategory(ModuleMetal.Config.CATEGORY_ITEM_INGOT).getBoolean(name)
        ...
        && this.config.getCategory(ModuleMetal.Config.CATEGORY_RECIPE_NUGGET_TO_INGOT).getBoolean(name);
  }
