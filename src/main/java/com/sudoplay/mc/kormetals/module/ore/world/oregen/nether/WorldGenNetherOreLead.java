...
public class WorldGenNetherOreLead extends
  ...
  private static class Config extends
    ...
    public Config() {
      super(
          ...
          new DimensionProfile(-1, 12, "minecraft:netherrack", new MinMaxInt(8, 12), new MinMaxInt(0, 128))
      );
    }
  }
  ...
  public WorldGenNetherOreLead(
      Kor kor,
      @KorJsonConfig(path = ModuleOre.Config.OreGenNether.CONFIG_PATH, file = "lead.json") Config config
  ) {
    super(
        config,
        kor.get(BlockNetherOre.class).getDefaultState().withProperty(BlockNetherOre.TYPE, MetalType.Lead)
    );
  }
}
