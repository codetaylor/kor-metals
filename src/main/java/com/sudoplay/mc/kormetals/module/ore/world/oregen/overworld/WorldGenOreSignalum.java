...
public class WorldGenOreSignalum extends
  ...
  public WorldGenOreSignalum(
      Kor kor,
      @KorJsonConfig(path = ModuleOre.Config.OreGenOverworld.CONFIG_PATH, file = "signalum.json") Config config
  ) {
    super(
        config,
        kor.get(BlockOre.class).getDefaultState().withProperty(BlockOre.TYPE, MetalType.Signalum)
    );
  }
}
